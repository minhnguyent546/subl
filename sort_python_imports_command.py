...
class SortPythonImportsCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit: sublime.Edit): 
        ...
        if not regions:
            return
        # import statements can have comment associated with them, so we need to expand the region
        ...
        for import_section_region in reversed(import_section_regions):
            ...
            tree.body = sorted(tree.body, key=cmp_to_key(node_cmp))
            ...
            sorted_section_content = unparse_imports(tree)
            self.view.replace(
                edit,
                sublime.Region(import_section_region[0].begin(), import_section_region[-1].end()),
...
def unparse_imports(tree: ast.Module, tab_size: int = 4) -> str:
    source = []
    tabs = " " * tab_size
